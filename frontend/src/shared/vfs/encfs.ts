...
import type { EncryptedOpEnvelope } from './opCrypto'
...
/** Seal an op into transport bytes (envelope framing over {@link sealOp}). */
export async function sealOpBytes(op: Op, key: KeyHandle): Promise<Uint8Array> {
  return encodeEnvelope(await sealOp(op, key))
}

/** Open transport bytes back into an op ({@link openOp} over the framing). */
export async function openOpBytes(bytes: Uint8Array, key: KeyHandle): Promise<Op> {
  const { meta, ciphertext } = peekEnvelope(bytes)
  return openOp({ ...meta, ciphertext }, key)
}
...
export class EncryptedFS implements SpaceFS {
  ...
  async sync(): Promise<void> {
    ...
    for (const s of stored) {
      ...
      const op = await openOpBytes(s.blob, this.key)
      ops.push(op)
      if (op.lamport > batchMax) batchMax = op.lamport
    }
    if (ops.length > 0) {
      this.replica.applyAll(ops)
  ...
  async writeCheckpoint(): Promise<void> {
    const cp: CheckpointData = { lamport: this.maxLamport, nodes: this.replica.materialize() }
