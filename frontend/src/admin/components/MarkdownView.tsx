...
export function MarkdownView({
  ...
  // flushes whatever is left so paper never shows a half-coloured document.
  useEffect(() => {
    const article = articleRef.current
    if (!article) return
    const run = highlightCodeBlocks(article)
    const onBeforePrint = () => { void run.flush() }
  ...
  const renderedMarkdown = useMemo(() => (
        ...
        <ReactMarkdown
          ...
          rehypePlugins={[
            ...
            rehypeKatex,
            // NO syntax highlighting here on purpose: it used to be the single
            // biggest cost of opening a page, and all of it sat between the
            // click and the first painted character. Blocks are coloured after
            // paint, in idle time — see the highlight effect below.
          ]}
          components={{
            ...
            a: ({ href, children, className, ...rest }) => {
              ...
              // A single leading slash is an in-app route. `//host/…` is NOT:
              // it is protocol-relative, i.e. off-origin, and the sanitizer
              // lets it through (no colon = "relative"). Routing it through
              // <Link> would navigate the whole admin tab to that host — so
              // treat it like any other external URL, in a new tab.
